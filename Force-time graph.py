...
# Converting force to float from column 1
...
df_even.to_csv(f"{base_path}_Force_time.csv", index=False)
print(f"Saved interpolated data to {base_path}_Force_time.csv")
...
plt.xlabel('Time (s)')
plt.ylabel('Force (N)')
plt.title('Force vs. Time - Porcine Material') # change name according to trial name
plt.grid(True)
plt.tight_layout()
plt.show()
