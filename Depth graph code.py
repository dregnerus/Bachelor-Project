...
# Parameters (may change for each video)
...
start_frame_idx  = 82       # zero‐based index of the frame where tracking should begin
needle_length_mm = 80.0     # full needle length in mm (hub→tip)
zoom_factor      = 2        # how much to enlarge the display for clicking only
min_duration_s   = 14.0      # ensure the graph runs at least 1 second
pixel_error      = 3        # ± pixels reading uncertainty

# Reading starting frame (start_frame_idx)
...
# STEP 2) Show zoomed frame and click hub, tip, surface
h, w = frame.shape[:2]
...
# STEP 3) Calibration & initial indent
...
while True:
    ...
    # (b) Indentation – vertical distance only, zeroed at start
...
# … after computing mean_vel and std_vel …

# STEP 6) Plot with error bands, swapping legend/text positions
...
# Place the mean/std text in the upper right
